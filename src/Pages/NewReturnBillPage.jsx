...
const NewReturnBillPage = ({ 
  ...
  async function addNewBill(
    ...
    const createApi = {
      ...
      data: { ...bill, billId: newBillId, returningItems, id: existingBill._id },
    };
  ...
  const updateReturnAmount = (setBill, bill) => {
    setBill((prev) => ({
      ...prev,
      amountReturn: bill?.cashPay + bill?.upiPay - bill?.billAmountTotal + returnedAmount,
    }));
  };
  ...
  useEffect(
    ...
    // eslint-disable-next-line
    [bill.cashPay, bill.upiPay, bill.billAmountTotal, returnedAmount]
  );
    ...
    <>
      <div>
        <div className="billing-container">
          ...
          <Button
            sx={{ marginRight: '1rem' }}
            disabled={ bill.amountReturn < 0 || !Object.keys(returningItems).length}
            className="print-btn"
            onClick={() =>
...
export default NewReturnBillPage;
