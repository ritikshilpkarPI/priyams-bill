...
export const ExpiredItemPOTable: React.FC<ExpiredItemTableProps> = ({
  ...
  const dealerOptions: { value: string; label: string }[] = dealers
    .filter((d): d is any & { _id: string } => Boolean(d._id))
    .map((dealer) => ({
      value: dealer._id,
  ...
  /* === UPDATED === */
  // Build groupedItems so that duplicates (same item._id) collapse into one main row
  const groupedItems: Record<string, typeof items> = {};
  for (const entry of items) {
    const idKey = entry._id;
    if (!groupedItems[idKey]) {
      groupedItems[idKey] = [];
    }
    groupedItems[idKey].push(entry);
  }
  /* =============== */
    ...
    >
      <Flex py="lg" justify="center">
        <Text size="lg" weight={600}>
          ...
          >
            {expiryBatchBoxId ? expiryBatchBoxId : boxId}
          </Code>
        </Text>
      </Flex>
        ...
        <div className={classes.tableContainer} role="table" aria-label="Expired Items">
          <div role="rowgroup">
            ...
            {Object.entries(groupedItems).map(([itemId, entries]) => {
              ...
              // Use only the first entry in this group for the main row details:
              const firstEntry = entries[0];
              const { sku, itemBrandName, companyName } = firstEntry;

              // Combine all itemShelfDates across every entry in this group:
              const allShelfDates = entries.flatMap((e) => e.itemShelfDates || []);

              // Pull Redux‐level expiry batches for this itemId
              const reduxItemExpiryBatches = expiryBatchMap[itemId] || [];
              const reduxMap = Object.fromEntries(
                reduxItemExpiryBatches.map((b) => [b.shelfId, b])
              );

              // Convert each shelf‐date into a row object:
              const allBatches = allShelfDates.map((shelf: any) => ({
                _id: shelf._id,
                            ...
                            >
                              <thead>
                                <tr>
                                  ...
                                  <th
                                    className={`${classes.nestedHeader} ${classes.nestedCellNumeric}`}
                                  >
                                    CP
                                  </th>
                              ...
                              <tbody>
                                {mergedBatches.map((batch) => {
                                  ...
                                  return (
                                    <tr
                                      ...
                                      className={rowClass}
                                      onClick={(e) =>
                                        !isEdit && startEdit(e, itemId, batch as any)
                                      }
                                    >
                                      <td
                                        className={
                                      ...
                                      <td
                                        className={`${classes.nestedCell} ${classes.nestedCellNumeric}`}
                                      >
                                        {isEdit ? (
                                          <NumberInput
                                            size="xs"
                                      ...
                                      <td className={classes.nestedCell}>
                                        {isEdit ? (
                                          <TextInput
                                            size="xs"
                                            type="date"
                                            value={dayjs(draft?.manufacturingDate).format(
                                              'YYYY-MM-DD'
                                            )}
                                            onClick={(e) => e.stopPropagation()}
                                            onChange={(e) =>
                                      ...
                                      <td className={classes.nestedCell}>
                                        {isEdit ? (
                                          <TextInput
                                            size="xs"
                                            type="date"
                                            value={dayjs(draft?.expiryDate).format(
                                              'YYYY-MM-DD'
                                            )}
                                            onClick={(e) => e.stopPropagation()}
                                            onChange={(e) =>
                                        ...
                                        {isEdit && (
                                          <>
                                            ...
                                            <ActionIcon
                                              ...
                                              variant="filled"
                                              onClick={(e) =>
                                                cancelEdit(e, batch._id, itemId, false)
                                              }
                                              title="Delete"
                                            >
                                              <IconX />
                                        ...
                                        {!isEdit && (
                                          <>
                                            <ActionIcon
                                              ...
                                              color="gray"
                                              onClick={(e) =>
                                                startEdit(e, itemId, batch as any)
                                              }
                                              title="Edit"
                                            >
                                              <svg
                                            ...
                                            <ActionIcon
                                              ...
                                              color="red"
                                              onClick={(e) =>
                                                cancelEdit(e, batch._id, itemId, true)
                                              }
                                              title="Delete"
                                            >
                                              <IconX size={16} />
                                ...
                                {addingItemId === itemId && (
                                  <tr
                                    className={`${classes.mainRow} ${classes.nestedRow} ${classes.updatedRow}`}
                                  >
                                    {/* Add‐new‐batch row */}
                                    <td className={classes.nestedCell}>
                                      <TextInput
                                    ...
                                    <td className={classes.nestedCell}>
                                      <TextInput
                                        ...
                                        type="date"
                                        value={dayjs(
                                          newBatchDraft.manufacturingDate
                                        ).format('YYYY-MM-DD')}
                                        onClick={(e) => e.stopPropagation()}
                                        onChange={(e) =>
                                          setNewBatchDraft((d) => ({
                                    ...
                                    <td className={classes.nestedActionCell}>
                                      <ActionIcon
                                        ...
                                        title="Save"
                                        loading={
                                          isAddingNewBatch && addingItemId === itemId
                                        }
                                      >
                                        <IconCheck />
                                      </ActionIcon>
