...
export const PurchaseOrderSummary = () => {
  ...
  const validateDealerDetails = async (purchaseOrder: PurchaseOrderDataType) => {
     try {
 ...
 const validatePaymentDetails = async (purchaseOrder: PurchaseOrderDataType) => {
    try {
      const paymentDetailsValidation = Yup.array().of(paymentDetailFormValidation);
 ...
 const enableDraftBtn = isValidDealerDetails && isValidItemDetails && isValidPaymentDetails && isBillImagesUploaded && !purchaseOrder.isDraft;
  return (
    <Flex align="left" gap="16px" direction="column" sx={{ border: "1px solid grey", padding: "16px", borderRadius: "8px", textAlign: "left", overflow: "scroll" }} mx="sm" mt="16px">
      <Title order={4} weight={400} align="left">
        You need to fill all the below forms to draft/approve this purchase order
      </Title>
