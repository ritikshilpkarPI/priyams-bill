function formatDate(dateStr?: string): string {
  ...
  export function generateStoreCSV(data: any[]): string {
    const headers = [
      'ID',
      'Barcode',
      'Name',
    ...
    data.forEach((item) => {
      ...
      if (shelves.length === 0) {
        ...
        const firstRow = [
          ...
          firstShelf?.initialStockQuantity ?? '',
          firstShelf?.currentStockQuantity ?? ''
        ].map((field) => JSON.stringify(field ?? '')).join(',');
        ...
        for (let i = 1; i < shelves.length; i++) {
          ...
          const shelfRow = [
            ...
            shelf?.initialStockQuantity ?? '',
            shelf?.currentStockQuantity ?? ''
          ].map((field) => JSON.stringify(field ?? '')).join(',');
