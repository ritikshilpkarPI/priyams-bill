...
const savePayment = async (req,res) =>{
  ...
  const updateSavedPayment = async(req,res)=>{
    try{
      ...
      const updatedOrder = await purchaseOrder.updateOne({
        ...
        totalPaidAmount:(Number(purchaseOrder.totalPaidAmount)+Number(payment.paidAmount)).toFixed(2)
    });
      res.status(200).send({message:"order added successfully",success:true,order:purchaseOrder,updatedOrder})
    }catch(err){
      res.status(400).send({message:err,success:false})
    }
  }
  const deletePaymentById = async(req,res) =>{
    try{
      ...
      const purchaseDetails = purchaseOrder.purchaseDetails.filter((detail,i) => i!== index);
      let totalPaidAmount = 0 ;
      purchaseDetails.forEach((payment) => {
        totalPaidAmount = totalPaidAmount + payment.paidAmount;
      })
      totalPaidAmount = Number((Math.round(totalPaidAmount * 100) / 100).toFixed(2))
      const updatedOrder = await purchaseOrder.updateOne({purchaseDetails,totalPaidAmount});
  ...
  const updatePaymentById = async(req,res)=>{
    try{
      ...
      const purchaseOrder = await PurchaseOrder.findById(purchase_id);
      const purchaseDetails = [...purchaseOrder.purchaseDetails.filter((order,i)=> i!=index),payment];
      let totalPaidAmount = 0;
      purchaseDetails.forEach((payment) => {
        totalPaidAmount = totalPaidAmount + payment.paidAmount;
      })
