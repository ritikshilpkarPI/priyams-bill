...
const getStaff = async (request, response) => {
    try {
        ...
        response.status(200).json({ status: true, message: 'details sent!', details: allStaffDetails });
    } catch (error) {
...
const addStaff = async (request, response) => {
    try {
        ...
        if (!name || !username || !role || !password) {
            return response.status(200).json({ status: false, message: 'please provide all details!' });
        }

        const staffDetail = await new Staff(request.body);
        staffDetail.save();
        response.status(200).json({ status: true, message: 'staff added!', staffDetail });
    } catch (error) {
        response.status(500).json(error);
    }
}
...
const updateStaff = async (request, response) => {
    try {
        ...
        const staffUpdate = await Staff.findOneAndUpdate({username: username}, request.body);
        response.status(200).json({ status: true, message: 'staff updated!', updatedItem: staffUpdate });
    } catch (error) {
        response.status(500).json(error);
    }
}
